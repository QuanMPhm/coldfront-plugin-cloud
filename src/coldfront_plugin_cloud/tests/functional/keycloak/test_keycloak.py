...
from coldfront_plugin_cloud import tasks, kc_client, attributes
...
class TestKeyCloakUserManagement(base.TestBase):
    ...
    def setUpTestData(cls) -> None:
        ...
        ResourceAttribute.objects.get_or_create(
            resource_attribute_type=ResourceAttributeType.objects.get(
                name=attributes.RESOURCE_KEYSTONE_GROUP_TEMPLATE
            ),
            resource=cls.resource,
            value="$resource_name/$project_name",
        )

    def new_user(self, username=None, add_to_keycloak=True) -> User:
        user = super().new_user(username)
        if add_to_keycloak:
            self.kc_admin_client.create_user(user.username)
        return user
    ...
    def test_user_added_to_allocation(self):
        ...
        # Group name determined by the RESOURCE_KEYSTONE_GROUP_TEMPLATE attribute, set to "$resource_name/$project_name" in tests
        user_groups = self.kc_admin_client.get_user_groups(user_id)
        self.assertIn(f"{self.resource.name}/{project.title}", user_groups)
    ...
    def test_user_removed_from_allocation(self):
        ...
        user_groups = self.kc_admin_client.get_user_groups(user_id)
        self.assertNotIn(f"{self.resource.name}/{project.title}", user_groups)
    ...
    def test_user_not_in_keycloak_added_to_allocation(self):
        ...
        group_id = self.kc_admin_client.get_group_id(
            f"{self.resource.name}/{project.title}"
        )
    ...
    def test_remove_one_user_keeps_others_in_group(self):
        ...
        self.assertNotIn(f"{self.resource.name}/{project.title}", user1_groups)

        user2_id = self.kc_admin_client.get_user_id(users[1].username)
        user2_groups = self.kc_admin_client.get_user_groups(user2_id)
        self.assertIn(f"{self.resource.name}/{project.title}", user2_groups)
    ...
    def test_user_in_multiple_allocations_groups(self):
        ...
        allocation2 = self.new_allocation(project2, self.resource, 1)
        ...
        user_id = self.kc_admin_client.get_user_id(user.username)
        user_groups = self.kc_admin_client.get_user_groups(user_id)
        self.assertIn(f"{self.resource.name}/{project1.title}", user_groups)
        self.assertIn(f"{self.resource.name}/{project2.title}", user_groups)
        ...
        user_groups = self.kc_admin_client.get_user_groups(user_id)
        self.assertNotIn(f"{self.resource.name}/{project1.title}", user_groups)
        self.assertIn(f"{self.resource.name}/{project2.title}", user_groups)
